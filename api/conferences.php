...
try {
    ...
    switch ($method) {
        case 'GET':            // Lấy ID từ URL nếu có
            ...
            if ($id > 0) {
                // Kiểm tra xem có phải request lấy speakers không
                if (isset($_GET['speakers'])) {
                    ...
                    if ($speakers) {
                        ...
                        echo json_encode([
                            'status' => false,
                            'message' => 'Không tìm thấy diễn giả cho hội nghị ID: ' . $id
                        ]);
                    }
                    exit;
                }

                // Kiểm tra xem có phải request lấy schedule không
                if (isset($_GET['schedule'])) {
                    ...
                    if ($schedule) {
                        ...
                        echo json_encode([
                            'status' => false,
                            'message' => 'Không tìm thấy lịch trình cho hội nghị ID: ' . $id
                        ]);
                    }
                    exit;
                }
